...
public class JEIPlugin implements IModPlugin {
    ...
    public static final RecipeType<CrabTrapWrapper> CRAB_TRAP_RECIPE = RecipeType.create(CrabbersDelight.MODID, "crab_trap_bait", CrabTrapWrapper.class);
    ...
    public List<CrabTrapWrapper> addWrappers() {
        List<CrabTrapWrapper> list = new ArrayList<>();
        for (ItemStack item : ForgeRegistries.ITEMS.getValues().stream().map(ItemStack::new).toList()) {
            if (item.is(CDModTags.CRAB_TRAP_BAIT)) {
                ...
                if (ForgeRegistries.ITEMS.tags().isKnownTagName(outputTag)) {
                    list.add(new CrabTrapWrapper(item, Ingredient.of(outputTag)));
                }
